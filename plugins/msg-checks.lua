...
local lang = redis:get(hash)
  ...
  tdcli.sendMessage(msg.to.id, "", 0, "⛥┇هاذه المجموعه ليست ضمن مجموعات البوت\n⛥┇راسل المطور لاضافه البوت @TAHAJ20", 0, "md")
...
del_msg(chat, tonumber(msg.id))
  ...
  tdcli.pinChannelMessage(msg.to.id, pin_msg, 1)
   ...
   tdcli.unpinChannelMessage(msg.to.id)
    ...
    if lang then
     tdcli.sendMessage(msg.to.id, msg.id, 0, '<b>⛥┇الايدي :</b> <code>'..msg.from.id..'</code>\n⛥┇المعرف :</b> '..('@'..msg.from.username or '<i>لا يوجد</i>')..'\n<i>⛥┇عذرا التثبيت مقفل لا يمكنك التثبيت الان</i>', 0, "html")
...
local link_caption = msg.media.caption:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]/") or msg.media.caption:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Dd][Oo][Gg]/") or msg.media.caption:match("[Tt].[Mm][Ee]/") or msg.media.caption:match("[Tt][Ee][Ll][e[Ss][Cc][Oo].[Pp][Ee]/") or msg.media.caption:match("[Tt][Ee][Ll][Ee][Gg][Tt][Aa][Mm].[Dd][Oo][Gg]/")
if link_caption
...
local link_msg = msg.text:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]/") or msg.text:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Dd][Oo][Gg]/") or msg.text:match("[Tt].[Mm][Ee]/") or msg.text:match("[Tt][Ee][Ll][e[Ss][Cc][Oo].[Pp][Ee]/") or msg.media.caption:match("[Tt][Ee][Ll][e[Ss][Cc][Oo].[Pp][Ee]/") or msg.media.caption:match("[Tt][Ee][Ll][Ee][Gg][Tt][Aa][Mm].[Dd][Oo][Gg]/")
if link_msg
and lock_link == "yes" then
 if is_channel then
 del_msg(chat, tonumber(msg.id))
...
return
   ...
   del_msg(chat, msg.id)
    kick_user(user, chat)
   if not lang then
  tdcli.sendMessage(chat, msg.id, 0, "🎐| _المستخدم 👤_ ["..user_name.."] \n📝| _الايدي 💫_ *["..data.id_.."]*\n📛|` _has been_ *kicked* _because of_ *flooding*", 0, "md")
   elseif lang then
  tdcli.sendMessage(chat, msg.id, 0, "🎐| _المستخدم 👤_ ["..user_name.."] \n📝| _الايدي 💫_ *["..data.id_.."]*\n📛|_العضو قام بعمل تكرار في المجموعه وتم طرده_", 0, "md")
    end

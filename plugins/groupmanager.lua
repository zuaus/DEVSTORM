...
data[tostring(msg.to.id)]["settings"]["link"] = "بالتحذير"
 save_data(_config.moderation.data, data)
   if not lang then
    return "_This group does not allow_ *Link messages.*\n_Users will be_ *warned* _first, then_ *kicked* _for second infringements"
    else
    return "👁‍🗨￤مرحــبا عزيــزي المستخــدم \n👁‍🗨￤تم قفل  الروابط \n👁‍🗨￤ خاصيه التحذير ⚠"
   end
...
data[tostring(msg.to.id)]["settings"]["spam"] = "بالتحذير"
 save_data(_config.moderation.data, data)
   if not lang then
    return "*Anti Spam* _protection already_ *enabled*.\n_Offender will be_ *warned* _first, then_ *kicked* _for second infringements"
    else
    return "👁‍🗨￤مرحــبا عزيــزي المستخــدم \n👁‍🗨￤تم قفل الكلايش \n👁‍🗨￤ خاصيه التحذير ⚠"
   end
elseif (matches[2] == "بالحذف" and not Clang) or (matches[2] == "بالحذف" and Clang) then
...
data[tostring(msg.to.id)]["settings"]["sticker"] = "بالتحذير"
 save_data(_config.moderation.data, data)
   if not lang then
    return "*Stickers* _already_ *prohibited*.\n_Sender will be_ *warned* _first, then_ *kicked* _for second violation."
    else
    return "👁‍🗨￤مرحــبا عزيــزي المستخــدم \n👁‍🗨￤تم قفل الملصقات \n👁‍🗨￤ خاصيه التحذير ⚠"
   end
elseif (matches[2] == "بالحذف" and not Clang) or (matches[2] == "بالحذف" and Clang) then
...
local target = msg.to.id
if ((matches[2] == "البوتات" and not Clang) or (matches[2] == "دخول البوتات" and Clang)) then
return lock_bots(msg, data, target)
end
if ((matches[2] == "التثبيت" and not Clang) or (matches[2] == "التثبيت" and Clang)) and is_owner(msg) then
return lock_pin(msg, data, target)
end
if ((matches[2] == "الدخول" and not Clang) or (matches[2] == "دخول الاعضاء" and Clang)) then
return lock_join(msg, data, target)
end
if ((matches[2] == "الاشعارات" and not Clang) or (matches[2] == "قفل الاشعارات" and Clang)) then
...
return "_Bot Commands Language Set To:_ AR"
end
end
end

if (matches[1]:lower() == "انكلش" and Clang) and is_owner(msg) then
local hash = "cmd_lang:"
redis:del(hash)
   if lang then

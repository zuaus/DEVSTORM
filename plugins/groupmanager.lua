...
elseif lang then
 return "⛥┇بالفعـل تـم قفــل الماركـــدون\n`•••⛥•••••❂•••••⛥•••`\n👨‍💻┇بواسطة : { "..msg.from.id.." }\n" 
end
else
...
elseif lang then
return "⛥┇تـــم تثبيـــت الرسـالـــه"
end
...
elseif lang then
return "⛥┇تـــم الغـــاء تثبيـــت الرسـالـــه"
end
elseif not is_owner(msg) then
   return 
 end
...
if matches[1] == 'newlink' or matches[1] == 'تغير الرابط' and is_mod(msg) then
 ...
 tdcli.exportChatInviteLink(msg.to.id, callback_link, nil)
  ...
  return "⛥┇ُتــم حفـــظ الـــقوانيــن 📇"
   end
  end
...
tdcli_function ({
  ...
  if matches[1] == 'setflood' or matches[1] == 'ضع تكرار' and is_mod(msg) then
			if tonumber(matches[2]) < 1 or tonumber(matches[2]) > 50 then
				return "⛥┇يـــمكنك وضـــع عــدد التــكرار مــن `{  1 / 50  }` فقَُـــط "
      end
    ...
    return "_Group_ *flood* _sensitivity has been set to :_ *[ "..matches[2].." ]*"
		...
		if matches[1]:lower() == 'clean' or matches[1] == 'مسح' and is_owner(msg) then
            ...
            if not lang then
                ...
                return "⛥┇لا يـــوجــد ادمنيـــه لــيتم مسحهَُـــم "
				end
            end
     ...
     if not lang then
					...
					return "⛥┇لاتـوجــد كلمـــات ليتـــم مسحهـــا"
             end
				end
       ...
       if not lang then
				...
				return "⛥┇ قـائـــمه الـــمنع  تـــم مسحهـــا"
           end
			end
            ...
            if not lang then
               ...
               return "⛥┇لا يـــوجــد قـوانـــين ليتـــم مســـحها"
             end
				end
            ...
            return "⛥┇تـــم مســـح القـوانـــين "
			end
       end
            ...
            if not lang then
               ...
               return "⛥┇لا يـــوجــد ترحـــيب فـي المــجمـوعـــه"
             end
				end
            ...
            return "⛥┇تــم مســح ترحـــيب  المــجمـوعـــه"
			end
       end
        ...
        if msg.to.type == "chat" then
            ...
            if not lang then
              ...
              return "⛥┇لا يـــوجــد وصــف فـي المــجمـوعـــه"
          end
				end
   ...
   tdcli.changeChannelAbout(chat, "", dl_cb, nil)
             ...
             if not lang then
              ...
              return "⛥┇تــم مســح وصـــف المــجمـوعـــه"
             end
		   	end
        end
		if matches[1]:lower() == 'clean' or matches[1] == 'مسح' and is_admin(msg) then
			if matches[2] == 'owners' or matches[2] == 'المدراء' then
             ...
             if not lang then
                ...
                return "⛥┇لا يـــوجــد مــدراء فـي المــجمـوعـــه"
            end
				end
            ...
            return "⛥┇تـــم تنزيـــل جميـــع الــمدراء "
          end
			end
     end
...
if lock_id == "🔒" then
return '⛥┋بالـتاكيـد تـــم تشــغيــل الايـــدي \n`•••⛥•••••❂•••••⛥•••`\n👨‍💻┇بواسطة : { '..msg.from.id.." }\n" 
else
data[tostring(chat)]['settings']['lock_id'] = "🔒"
save_data(_config.moderation.data, data)
return '⛥┋ تــم تشــغيــل الايـــدي \n`•••⛥•••••❂•••••⛥•••`\n👨‍💻┇بواسطة : { '..msg.from.id.." }\n" 
end
end
end
...
lock_id = data[tostring(chat)]['settings']['lock_id']
if lock_id == "🔓" then
return '⛥┋بالـتاكيـد تـــم ايقــــاف الايـــدي \n`•••⛥•••••❂•••••⛥•••`\n👨‍💻┇بواسطة : { '..msg.from.id.." }\n" 
else
data[tostring(chat)]['settings']['lock_id'] = "🔓"
save_data(_config.moderation.data, data)
return '⛥┋تـــم ايقــــاف الايـــدي \n`•••⛥•••••❂•••••⛥•••`\n👨‍💻┇بواسطة : { '..msg.from.id.." }\n" 
end
end
end
----------------الترحيب-----------------------
...
if matches[1] == "الترحيب"  and is_mod(msg) then
		...
		return "⛥┇نــورت المــجموعــه حياتــي  \n[⛥┇اضغط هنا لدخول الى جديدنا](http://telegram.dog/TEAMSTORM)"
	end
	end
-----------------------------------------
